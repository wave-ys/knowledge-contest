<template>
  <div>
    ...
    <div :id="'anchor' + 0" :style="{ height: user ? '0px' : '46px' }"></div>
    <van-notice-bar
      :scrollable="false"
      :text="
        (user ? `${userInfo.name}同学的` : '您的') +
          `成绩：${completePaper.score} / ${constants.CHOICE_QUESTION_NUM *
            constants.CHOICE_QUESTION_SCORE +
            constants.JUDGE_QUESTION_NUM * constants.JUDGE_QUESTION_SCORE}`
      "
      v-loading="loading"
    />
    <div class="details" v-loading="loading">
      <h2 class="details-header">{{ user ? "" : "答题详情" }}</h2>
      <div
        :class="[
          'card',
...
export default {
  ...
  async created() {
    this.loading = true;
    this.completePaper = await paperApi.getCompletePaper(this.userInfo.id);
    this.loading = false;
  },
  watch: {
    async user(val) {
      this.loading = true;
      this.completePaper = await paperApi.getCompletePaper(val.id);
      this.loading = false;
    }
  }
};
